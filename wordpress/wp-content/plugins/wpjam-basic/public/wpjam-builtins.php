	...
	public static function filter_post_single_row($single_row, $post_id){
		...
		if(wpjam_basic_get_setting('post_list_set_thumbnail', 1) && post_type_supports($ptype, 'thumbnail')){		
			...
			if(current_user_can('edit_post', $post_id)){
				$thumbnail	= wpjam_get_list_table_row_action('set', ['id'=>$post_id, 'class'=>'wpjam-thumbnail-wrap', 'title'=>$thumbnail]);
			}
	...
	public static function filter_term_single_row($single_row, $term_id){
		...
		if(WPJAM_List_Table_Action::get('set_thumbnail')){
			...
			if(current_user_can(get_taxonomy($taxonomy)->cap->edit_terms)){
				$thumbnail = wpjam_get_list_table_row_action('set_thumbnail', ['id'=>$term_id, 'class'=>'wpjam-thumbnail-wrap', 'title'=>$thumbnail]);
			}
